...
gen_mod_input <- function(build_dens, build_prox, build_terrain) {
    terrain_df <- as.data.frame(build_terrain) %>%
      select(UID, elevation, slope, aspect)
    ...
    mod_input <- as.data.frame(build_prox) %>%
      mutate(lon = st_coordinates(st_transform(st_centroid(geometry), crs = 4326))[,1],
             ...
             utm_x = st_coordinates(st_centroid(geometry))[,1],
      ...
      left_join(terrain_df, by = "UID") %>%
      select(UID, lon, lat, utm_x, utm_y, DAMAGE = DAMAGE.x, distance_to_nearest_building = dst2n_b, bearing_to_nearest_building = brng_t_c_, distance_to_nearest_tree = dst2n_t, bearing_to_nearest_tree = brng_t_n_, number_of_trees_2m = nm_tr_2, number_of_trees_5m = nm_tr_5, number_of_trees_10m = nm_t_10, area_of_trees_2m, area_of_trees_5m, area_of_trees_10m, area_ext_build_2 = ar_x__2, area_ext_build_10 = ar___10, area_ext_build_25 = ar___25, area_ext_build_50 = ar___50, area_ext_build_100 = a___100, tree_dens, build_dens, elevation, slope, aspect)
...
option_list <- list(
  ...
  make_option(c("-T","--build_terrain"),type="character", help="Path to building terrain"),
...
build_terrain <- st_read(opt$build_terrain, quiet = T)

mod_input <- gen_mod_input(build_dens = build_dens, build_prox = build_prox, build_terrain = build_terrain)
