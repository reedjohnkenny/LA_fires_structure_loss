...
chm_agg <- terra::aggregate(palisades_pre_chm, fact = 4)
  chm_crop <- terra::crop(chm_agg, terra::vect(pal_study_bound), mask = T)
  chm_crop_mask <- terra::mask(chm_crop, terra::vect(pal_buildings), inverse = T)
  ...
  crowns_polys_ndvi <- terra::extract(maxpre_ndvi_rast, crowns_polys, fun = mean, bind = T) %>%  
    st_as_sf() %>%  
    select(focal_median, maxar_pre_ndvi = ms8) 

ggplot(crowns_polys_ndvi, aes(x = maxar_pre_ndvi)) +
  geom_histogram() +
  theme_classic()

ggsave("../Supp_Figures/pal_hist.png")
```
...
chm_agg <- terra::aggregate(eaton_pre_chm, fact = 4)
  chm_crop <- terra::crop(chm_agg, terra::vect(eaton_burn_bound), mask = T)
  chm_crop_mask <- terra::mask(chm_crop, terra::vect(eaton_buildings), inverse = T)
  # Post-processing median filter
  kernel <- matrix(1,3,3)
  chm_sm <- terra::focal(chm_crop_mask, w = kernel, fun = median, na.rm = TRUE)
  ...
  crowns_polys_ndvi <- terra::extract(eaton_pre_ndvi_rast, crowns_polys, fun = mean, bind = T) %>%  
    st_as_sf() %>%  
    select(focal_median, maxar_pre_ndvi = ms8) 
...
ggsave("../Supp_Figures/eaton_hist.png")
