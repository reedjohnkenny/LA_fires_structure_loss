...
fprints <- st_read("../tmp_data/eaton_burned_fprints.shp") %>% st_transform(crs = 32611) %>% 
...
print_buff <- st_buffer(print_crop, 2)

all_crop <- rbind(tree_crop, print_crop)

vis_crop <- terra::crop(max_vis, rast_rect, mask = T)
...
ex_polys <-  tm_shape(vis_crop) +
 tm_rgb() +
  ...
  tm_shape(print_buff) +
  tm_polygons(fill = NULL, col = "orange") +
   tm_compass(type = "4star", position = c("left", "bottom"), frame = T, bg = T, bg.color = "white") +
  tm_scalebar(breaks = c(0, 0.01, 0.02), text.size = 0.5, position = c("center", "bottom"), bg = T, bg.color = "white", frame = T) +
  tm_title("Example Polygons", position = c("top", "center"), frame = TRUE, bg.color = "white", bg = T) +
...
cols <- c("distance_to_nearest_tree", "distance_to_nearest_building", "angular_diff_tree", "number_of_trees_2m", "number_of_trees_5m", "number_of_trees_10m",  "area_of_trees_2m",  "area_of_trees_5m",  "area_of_trees_10m", "area_ext_build_2", "area_ext_build_10", "area_ext_build_25", "area_ext_build_50", "area_ext_build_100", "angular_diff_build", "build_dens", "tree_dens", "mean_perc_struc", "area_tree_200m", "total_build_200_m", "fprint_area", "num_trees_200m", "mean_tree_density", "mean_struc_density", "num_builds_200m", "tree_dens_500", "area_tree_100m", "area_tree_50m", "total_build_100_m", "build_dens_500", "total_build_50_m")
...
eaton_imp <- plot_effects_mag(eaton_scaled_m, label_map = label_map, bar_fill = "black", plot_title = "A. Predictor Importance: Eaton") + theme(axis.text.y.left = element_markdown())
...
cols <- c("distance_to_nearest_tree", "distance_to_nearest_building", "angular_diff_tree", "number_of_trees_2m", "number_of_trees_5m", "number_of_trees_10m",  "area_of_trees_2m",  "area_of_trees_5m",  "area_of_trees_10m", "area_ext_build_2", "area_ext_build_10", "area_ext_build_25", "area_ext_build_50", "area_ext_build_100", "angular_diff_build", "build_dens", "tree_dens", "mean_perc_struc", "area_tree_200m", "total_build_200_m", "num_trees_200m", "mean_tree_density", "mean_struc_density", "num_builds_200m", "tree_dens_500", "area_tree_100m", "area_tree_50m", "total_build_50_m")
...
```{r}
int_plots <- arrangeGrob(eaton_tree_int, pal_tree_int, layout_matrix = rbind(c(1,1,1,2,2,2,2)))

ggsave("../Figures/int.png", int_plots, width = 6, height = 2, units = "in", dpi = 300)
...
trees_dist <- plot_pred_change(Eaton_struc_spamm_1, Palisades_struc_spamm_1, var = "distance_to_nearest_tree",
                 labels = c("Eaton", "Palisades"), delta = 1) + 
  ggtitle("D.") +
...
marg_plots <- arrangeGrob(struc_dens, tree_dens, trees_2, trees_dist, dist, build_2, legend, 
...
cols <- names(Eaton_data[,2:7])

col_labels <- label_map[cols]

names(Eaton_data)[match(cols, names(Eaton_data))] <- unname(col_labels)

Eaton_data_long <- Eaton_data %>% pivot_longer(cols = 2:7) %>%
  mutate(
    destroyed = factor(
      destroyed,
      levels = c(1, 0),                # adjust if your coding is reversed
      labels = c("destroyed", "not-destroyed")
    )
  )



ggplot(Eaton_data_long, aes(x = destroyed, y = value, fill = destroyed)) +
...
ggsave("../Supp_Figures/eaton_sum_stats.png", width = 6, height = 4, units = "in")
...
ggplot(Eaton_data, aes(x = `Structure density (ha<sup>-1</sup>)`, y = `Tree density (ha<sup>-1</sup>)`, color = destroyed)) + geom_point()
...
cols <- names(pal_data[,2:7])

col_labels <- label_map[cols]

names(pal_data)[match(cols, names(pal_data))] <- unname(col_labels)
...
ggplot(pal_data_long, aes(x = destroyed, y = value, fill = destroyed)) +
  geom_violin(trim = TRUE, alpha = 0.5) +
  facet_wrap(~ name, scales = "free_y") +
  scale_fill_manual(values = c("red", "forestgreen")) +
  theme_minimal(base_size = 13) +
...
ggsave("../Supp_Figures/pal_sum_stats.png", width = 6, height = 4, units = "in")
